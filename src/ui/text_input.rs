...
/// 注册输入组件依赖的快捷键；`ctrl` 面向 Windows/Linux，`cmd` 面向 macOS。
pub(crate) fn bind_input_keys(cx: &mut App) {
    // 动作类型各不相同，无法放入同一数组，逐条注册。
    let bindings = [
...
impl Element for TextElement {
    ...
    fn prepaint(
        ...
        let (display_text, text_color) = if content.is_empty() {
            (input.placeholder.clone(), hsla(0., 0., 0.5, 0.35))
        } else {
            (content, style.color)
        };
...
// 保留 Keystroke 导入引用：占位避免 unused 告警，后续输入过滤时使用。
#[allow(dead_code)]
fn _keystroke_marker(_: &Keystroke) {}
